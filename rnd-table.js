...
function lookupRNDFromImmoWertA(gnd, tableAge, points) {
  const p = Math.max(0, Math.min(20, Math.round(points)));
  if (Number(gnd) !== 80) return null;
  const age = Math.max(0, Math.min(80, Math.round(tableAge)));
  const row = RND_TABLE_GND_80[age] || RND_TABLE_GND_80[80];
  return {
    rnd: row[p],
    source: `ImmoWertA Anlage 2 Tabelle b: GND 80, Tabellenalter ${age}, Modernisierungspunkte ${p}`,
  };
}
const originalCalculateModifiedRND = calculateModifiedRND;
calculateModifiedRND = function (d) {
  const gnd = Math.max(1, Number(d.totalUsefulLife) || 80),
    realAge = buildingAge(d),
    tableAge = Math.min(realAge, gnd),
    baseRnd = Math.max(0, gnd - realAge),
    points = Math.max(0, Math.min(20, modernizationPoints())),
    lookup = lookupRNDFromImmoWertA(gnd, tableAge, points);
  if (!lookup) return originalCalculateModifiedRND(d);
  return {
    gnd,
    realAge,
    tableAge,
    ...
    points,
    rnd: lookup.rnd,
    info: `RND transparent: Bewertungsjahr ${getYearFromDate(d.valuationDate)}; Baujahr ${d.constructionYear || '–'}; reales Gebäudealter ${realAge} J.; GND ${gnd} J.; Basis-RND = max(0, ${gnd} − ${realAge}) = ${baseRnd} J.; Tabellenalter = min(${realAge}, ${gnd}) = ${tableAge} J.; Modernisierung ${points}/20 Punkte; ${lookup.source}; angesetzte modifizierte RND = ${lookup.rnd} J.`,
  };
};
